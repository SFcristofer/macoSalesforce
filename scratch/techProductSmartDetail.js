import { LightningElement, api, wire, track } from 'lwc';
import { getRecordUi } from 'lightning/uiRecordApi';
import { ShowToastEvent } from 'lightning/platformShowToastEvent';

export default class TechProductSmartDetail extends LightningElement {
    @api recordId;
    @track sections = [];
    @track activeSections = [];
    @track isEditMode = false;
    @track isLoading = true;

    toggleEditMode() {
        this.isEditMode = !this.isEditMode;
    }

    handleSubmit(event) {
        event.preventDefault(); // Stop default submit
        const fields = event.detail.fields;

        // Limpiar los valores de las secciones que el usuario haya ocultado ("quitado")
        this.sections.forEach(sec => {
            if (sec.subGroups) {
                sec.subGroups.forEach(group => {
                    // Si el grupo no es visible y es removible, significa que se eliminó o está vacío
                    if (!group.isVisible && group.isRemovable) {
                        group.fields.forEach(f => {
                            fields[f.name] = null;
                        });
                    }
                });
            }
        });

        // Hacer el submit manualmente
        this.template.querySelector('lightning-record-edit-form').submit(fields);
    }

    handleSuccess(event) {
        this.dispatchEvent(
            new ShowToastEvent({
                title: 'Éxito',
                message: 'El registro ha sido actualizado correctamente.',
                variant: 'success'
            })
        );
        this.isEditMode = false;
    }

    handleError(event) {
        this.dispatchEvent(
            new ShowToastEvent({
                title: 'Error al Guardar',
                message: event.detail.detail || 'Revisa los campos para asegurar que cumplen con las reglas de validación.',
                variant: 'error'
            })
        );
    }

    handleAddMasterbox(event) {
        const secId = event.target.dataset.secId;
        const sec = this.sections.find(s => s.id === secId);
        if (sec) {
            const group = sec.subGroups.find(g => g.type === 'masterbox' && !g.isVisible);
            if (group) group.isVisible = true;
        }
    }

    handleAddProduct(event) {
        const secId = event.target.dataset.secId;
        const sec = this.sections.find(s => s.id === secId);
        if (sec) {
            const group = sec.subGroups.find(g => g.type === 'producto' && !g.isVisible);
            if (group) group.isVisible = true;
        }
    }

    handleRemoveGroup(event) {
        const secId = event.target.dataset.secId;
        const groupId = event.target.dataset.groupId;
        const sec = this.sections.find(s => s.id === secId);
        if (sec) {
            const group = sec.subGroups.find(g => g.id === groupId);
            if (group) {
                group.isVisible = false;
                
                // Vaciar inputs visualmente
                const inputs = this.template.querySelectorAll(`lightning-input-field`);
                inputs.forEach(input => {
                    if (group.fields.some(f => f.name === input.fieldName)) {
                        input.value = null;
                    }
                });
            }
        }
    }

    @wire(getRecordUi, { recordIds: '$recordId', layoutTypes: 'Full', modes: 'View' })
    wiredRecordUi({ error, data }) {
        if (data) {
            const layouts = data.layouts.Product2;
            const layoutIds = Object.keys(layouts);
            const objectInfo = data.objectInfos && data.objectInfos.Product2;
            const recordInfo = data.records && data.records[this.recordId];
            
            if (layoutIds.length > 0) {
                const layoutData = layouts[layoutIds[0]].Full.View;
                
                if (layoutData && layoutData.sections) {
                    this.sections = layoutData.sections.map((sec, index) => {
                        let fields = [];
                        
                        sec.layoutRows.forEach(row => {
                            row.layoutItems.forEach(item => {
                                item.layoutComponents.forEach(comp => {
                                    if (comp.apiName) {
                                        const apiName = comp.apiName;
                                        let isCustomLookup = false;
                                        let displayValue = '';
                                        let recordLink = '';
                                        let label = apiName;
                                        
                                        // Extraemos el Label oficial de la Metadata
                                        if (objectInfo && objectInfo.fields[apiName]) {
                                            label = objectInfo.fields[apiName].label;
                                        }

                                        // Verificamos si la UI API ya tiene el nombre traducido del ID para inyectarlo manualmente
                                        if (recordInfo && recordInfo.fields[apiName]) {
                                            const rawVal = recordInfo.fields[apiName].value;
                                            const dispVal = recordInfo.fields[apiName].displayValue;
                                            
                                            // Si el framework nos da un ID crudo pero tiene el Nombre para mostrar (dispVal), forzamos su renderizado
                                            if (rawVal && typeof rawVal === 'string' && rawVal.startsWith('00') && rawVal.length >= 15 && dispVal) {
                                                isCustomLookup = true;
                                                displayValue = dispVal;
                                                recordLink = `/${rawVal}`;
                                            }
                                        }

                                        fields.push({
                                            name: apiName,
                                            label: label,
                                            isCustomLookup: isCustomLookup,
                                            displayValue: displayValue,
                                            recordLink: recordLink,
                                            cssClass: apiName.toLowerCase().includes('recordtype') ? 'tech-no-click' : ''
                                        });
                                    }
                                });
                            });
                        });
                        
                        const heading = sec.heading || `Sección ${index}`;
                        const headingLower = heading.toLowerCase();
                        
                        const isMasterbox = headingLower.includes('masterbox') || headingLower.includes('caja');
                        const isProduct = headingLower.includes('dimension') || headingLower.includes('variante') || headingLower.includes('producto');
                        const isCard = isMasterbox || isProduct;
                        
                        let icon = '';
                        if (isMasterbox) icon = '📦 ';
                        if (isProduct) icon = '🛠️ ';

                        // CLASIFICAR CAMPOS EN SUB-GRUPOS
                        let baseFields = [];
                        let mbFields = { 2: [], 3: [], 4: [], 5: [], 6: [] };
                        let prFields = { 2: [], 3: [], 4: [], 5: [], 6: [] };

                        fields.forEach(f => {
                            let matchMb = f.name.match(/^Masterbox([2-6])/);
                            let matchPr = f.name.match(/^Producto([2-6])/);
                            
                            if (matchMb) mbFields[matchMb[1]].push(f);
                            else if (matchPr) prFields[matchPr[1]].push(f);
                            else baseFields.push(f);
                        });

                        let subGroups = [];
                        if (baseFields.length > 0) {
                            subGroups.push({ id: `base-${index}`, title: '', isVisible: true, isRemovable: false, fields: baseFields, type: 'base' });
                        }

                        let hasMb = false;
                        for (let i = 2; i <= 6; i++) {
                            if (mbFields[i].length > 0) {
                                hasMb = true;
                                let hasData = mbFields[i].some(f => recordInfo && recordInfo.fields[f.name] && recordInfo.fields[f.name].value != null && recordInfo.fields[f.name].value !== false && recordInfo.fields[f.name].value !== '');
                                subGroups.push({ id: `masterbox${i}`, title: `📦 Master Box ${i}`, isVisible: hasData, isRemovable: true, type: 'masterbox', fields: mbFields[i] });
                            }
                        }

                        let hasPr = false;
                        for (let i = 2; i <= 6; i++) {
                            if (prFields[i].length > 0) {
                                hasPr = true;
                                let hasData = prFields[i].some(f => recordInfo && recordInfo.fields[f.name] && recordInfo.fields[f.name].value != null && recordInfo.fields[f.name].value !== false && recordInfo.fields[f.name].value !== '');
                                subGroups.push({ id: `producto${i}`, title: `🛠️ Producto Adicional ${i}`, isVisible: hasData, isRemovable: true, type: 'producto', fields: prFields[i] });
                            }
                        }

                        return {
                            id: `sec-${index}`,
                            label: heading,
                            isCard: isCard,
                            icon: icon,
                            subGroups: subGroups,
                            hasAddMasterboxButton: hasMb,
                            hasAddProductButton: hasPr
                        };
                    });
                    
                    this.activeSections = this.sections.map(s => s.id);
                    this.isLoading = false;
                }
            }
        } else if (error) {
            console.error('Error fetching Layout via UI API', error);
            this.isLoading = false;
        }
    }
}
